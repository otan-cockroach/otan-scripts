permutations = [
    ...
    ('ON UPDATE RESTRICT', 'ON UPDATE SET NULL'),
    ('ON UPDATE RESTRICT', 'ON UPDATE CASCADE'),
    ('ON UPDATE CASCADE', 'ON UPDATE SET DEFAULT'),
    ('ON UPDATE CASCADE', 'ON UPDATE SET NULL'),
    ('ON UPDATE SET DEFAULT', 'ON UPDATE CASCADE'),
    ('ON UPDATE SET DEFAULT', 'ON UPDATE SET NULL'),
...
""".format(first, second, first, second))
    ...
    if 'SET DEFAULT' in first:
        ...
        print('statement error 123.*referenced.*table "t2"')
    else:
        assert False, "{}, {}".format(first, second)
